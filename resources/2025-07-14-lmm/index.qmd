...
- There are three types of variance model for $\mathbf{R}_i$ and $\mathbf{G}_i$:  
...
\begin{align*}
l &= \log f(\boldsymbol{y}, \boldsymbol{b} ; \boldsymbol{\sigma})\\
...
## Algorithm {style="font-size:0.9em;"}
...
6. Repeat 1-2, if $\ell_R^{(t)} - \ell_R^{(t-1)} < \text{threshold}$ the break.
